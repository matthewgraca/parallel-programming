...
__global__ void sum(float *d_a, float *d_b, float *d_c) {
    ...
    // only allow threads [0,5] row and [0,5] col to execute
    d_c[row * %(MATRIX_SIZE)s + col] = d_a[row * %(MATRIX_SIZE)s + col] + d_b[row * %(MATRIX_SIZE)s + col];
}
"""
...
# already allocated space and matrices on device and host
# already compiled kernel function
